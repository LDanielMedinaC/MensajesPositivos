...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView = findViewById(R.id.mensajeText);
        textView.setText(msj[index]);
        rnd = new Random();
        editText = findViewById(R.id.editText);
        Date date = new Date();
        ConstraintLayout constraintLayout = findViewById(R.id.backGround);
        if(date.getHours() > 6 && date.getHours() < 20) {
            constraintLayout.setBackgroundColor(Color.parseColor("#FFFBE6"));
        }else
            constraintLayout.setBackgroundColor(Color.parseColor("#B9E4C9"));
    }

    public void nextButton(View view){
        if(++index >= msj.length) index = 0;
        textView.setText(msj[index]);
    }

    public void backButton(View view){
        if(--index < 0) index = msj.length - 1;

        textView.setText(msj[index]);
    }

    public void rndButton(View view){
        index = rnd.nextInt(msj.length - 1);
        textView.setText(msj[index]);
    }

    public void goButton(View view){
        String idx = editText.getText().toString();
        if(idx.equals("")) {
            Toast.makeText(this, "introduce un numero", Toast.LENGTH_SHORT).show();
            return;
        }
        if(Integer.parseInt(idx) >= msj.length) {
            Toast.makeText(this, "Introduce un numero menor", Toast.LENGTH_SHORT).show();
            return;
        }
